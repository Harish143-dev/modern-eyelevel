      ...
      <section className="px-20 py-20 bg-background relative z-10">
        <div className="max-w-5xl mx-auto text-center flex flex-col items-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full text-left">
            {[
              { title: "PROFILE OPTIMISATION", desc: "Founder and leadership profile — headline, About, Featured section, and banner", icon: <UserCheck className="w-10 h-10 text-primary mb-6" /> },
              { title: "COMPANY PAGE", desc: "Full page build and an always-on content calendar", icon: <Building2 className="w-10 h-10 text-primary mb-6" /> },
              { title: "LINKEDIN CONTENT STRATEGY", desc: "Posts, carousels, and newsletters built around your positioning", icon: <FileText className="w-10 h-10 text-primary mb-6" /> },
              { title: "PROSPECTING & OUTREACH", desc: "Sales Navigator-assisted connection strategy and sequenced outreach", icon: <UserPlus className="w-10 h-10 text-primary mb-6" /> },
              { title: "LINKEDIN ADS", desc: "Sponsored content, message ads, and lead gen forms", icon: <Megaphone className="w-10 h-10 text-primary mb-6" /> },
              { title: "PIPELINE ATTRIBUTION", desc: "Monthly reporting that ties LinkedIn activity to real pipeline", icon: <LineChart className="w-10 h-10 text-primary mb-6" /> }
            ].map((card, idx) => (
              <motion.div
                key={idx}
                {...scrollAnimProps}
